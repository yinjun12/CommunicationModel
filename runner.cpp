...
OnlineService::OnlineService(double thsv, uint32_t time, int infectCnt, int uninfectCnt) 
	:m_threhsoldValue(thsv),m_time(time),m_onlineInfectCounts(infectCnt),
	m_onlineUnInfectCounts(uninfectCnt) {
}

int OnlineService::getInfectCounts() {
	return m_onlineInfectCounts;
}

int OnlineService::getUninfectCounts() {
	return m_onlineUnInfectCounts;
}

void OnlineService::setOnlineInfectPersons(std::vector<std::shared_ptr<Person>> onlineInfectPersons) {
	m_onlineInfectPersons = onlineInfectPersons;
}

void OnlineService::setOnlineUninfectPersons(std::vector<std::shared_ptr<Person>> onlineUninfectPersons) {
	m_onlineUninfectPersons = onlineUninfectPersons;
}
...
void OnlineService::infectCure() {
	for (auto record = m_onlineInfectPersons.begin(); record != m_onlineInfectPersons.end();) {
		...
		if ((stuNum.find("online-infect-") == std::string::npos)&&(*record)->m_cureTime == CURE_TIME) {
			(*record)->m_cureTime = 0;
