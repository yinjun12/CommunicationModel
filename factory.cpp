...
void PersonFactory::randomGeneratePerson() {
	...
	for (int i = 0; i < m_onlineCounts; i++) {
		m_vecOnlinePersons.push_back(randomGenerateOnePerson(PersonType::ONLINE_UNINFECT, i));
	}

	/*生成线下人物*/
	for (int i = 0; i < m_offlineCounts; i++) {
		m_vecOfflinePersons.push_back(randomGenerateOnePerson(PersonType::OFFLINE, i));
	}
}

std::shared_ptr<Person> BaseFactory::randomGenerateOnePerson(PersonType ptype,int i) {
	std::shared_ptr<Person>record = std::make_shared<Person>();
	switch (ptype) {
	case PersonType::ONLINE_INFECT :
			record->m_stuNum = std::string("online-infect-") + std::to_string(i + 1);
			record->m_online = true;
			record->m_infective = true;
			break;
	case PersonType::ONLINE_UNINFECT:
			record->m_stuNum = std::string("online-uninfect-") + std::to_string(i + 1);
			record->m_online = true;
			record->m_infective = false;
			break;
	case PersonType::OFFLINE:
			record->m_stuNum = std::string("offline-") + std::to_string(i + 1);//线下默认被感染
...
Location BaseFactory::randomGenerateLocation() {
	Location lo;
...
void OnlinePersonFactory::setInfectCounts(int infectCnts) {
	m_infectCounts = infectCnts;
}

void OnlinePersonFactory::setUninfectCounts(int uninfectCnts) {
	m_uninfectCounts = uninfectCnts;
}

void OnlinePersonFactory::randomGenerateOnliePerson() {
	/*生成线上感染人物*/
	m_infectPerson.reserve(m_infectCounts);
	for (int i = 0; i < m_infectCounts; i++) {
		m_infectPerson.push_back(randomGenerateOnePerson(PersonType::ONLINE_INFECT, i));
	}

	/*生成线上未感染人物*/
	m_uninfectPerson.reserve(m_uninfectCounts);
	for (int i = 0; i < m_uninfectCounts; i++) {
		m_uninfectPerson.push_back(randomGenerateOnePerson(PersonType::ONLINE_UNINFECT, i));
	}
}

std::vector<std::shared_ptr<Person>> OnlinePersonFactory::getInfectPersons() {
	return m_infectPerson;
}

std::vector<std::shared_ptr<Person>> OnlinePersonFactory::getUninfectPersons() {
	return m_uninfectPerson;
}

std::shared_ptr<BaseService> OnlinePersonFactory::accquireService() {
	std::shared_ptr<OnlineService> service = std::make_shared<OnlineService>(0.5,0,20,100);

	setInfectCounts(service->getInfectCounts());
	setUninfectCounts(service->getUninfectCounts());

	randomGenerateOnliePerson();

	service->setOnlineInfectPersons(getInfectPersons());
	service->setOnlineUninfectPersons(getUninfectPersons());
