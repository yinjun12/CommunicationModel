...
public:
	OnlineService(double thsv,uint32_t time, int infectCnt, int uninfectCnt);

	int getInfectCounts();
	int getUninfectCounts();
	...
	void setOnlineInfectPersons(std::vector<std::shared_ptr<Person>> onlineInfectPersons);
	void setOnlineUninfectPersons(std::vector<std::shared_ptr<Person>> onlineUninfectPersons);
};
